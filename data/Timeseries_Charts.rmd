...
Then we can recreate our timeseries chart, this time focusing on a shorter time period. Here we also shade in the background according to the stringency of response [(see other code in repo)](https://github.com/fcorowe/covid_energy/blob/master/methods/covid_cases_line_plot_2.R), and change the order of the cities according to when the pandemic began. 

```{r}
shade_select <- data_bydate %>% dplyr::select(City, Date, Response_Stringency)
inx <- c(FALSE, diff(shade$Response_Stringency) != 0)
shade_select <- shade_select[inx, ]
...
ggplot(data_bydate)+
  geom_line(aes(x=Date, y=Mobility_Workplace),size = 2, lineend="round", linejoin="round", color="Red")+
  geom_line(aes(x=Date, y=Mobility_Residential),size = 2, lineend="round", linejoin="round", color="Blue")+
  geom_rect(data = shade_select, ymin = -Inf, ymax = Inf, aes(NULL, NULL, xmin = start, xmax = end, fill = Response_Stringency), alpha = .2) +
  facet_wrap(~City)+
  labs(caption = " 
       Based on Google Mobility Reports 2020")+ 
  labs(title="Mobility: Workplace and residential", y="Change in mobility (%)")+
