...
```{r}
getwd()
setwd("C:/Users/Cait Robinson/OneDrive - The University of Liverpool/Projects/COVID_NightLights/Selected_Cities")
getwd()
```

**Install useful packages for your analysis:** Install a range of useful packages for creating graphs (ggplot2) and organising or manuipulating data ([tidyr](https://tidyr.tidyverse.org/), [dplyr](https://dplyr.tidyverse.org/)). Additionally, install [RColorBrewer](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3) for accessing pretty colour palettes and [lubridate](https://lubridate.tidyverse.org/) for working with dates. 
...
```{r}
ggplot(data)+
  geom_line(aes(x=Date, y=Mobility_Workplace, color=City),size=1, lineend="round", linejoin="round")+
  labs(caption = " 
...
```{r}
ggplot(data)+
  geom_line(aes(x=Date, y=Mobility_Workplace),size=1, lineend="round", linejoin="round", color="Red")+
  geom_line(aes(x=Date, y=Mobility_Residential),size=1, lineend="round", linejoin="round", color="Black")+
  facet_wrap(~City)+
  labs(caption = " 
       Based on Google Mobility Reports 2020")+ 
