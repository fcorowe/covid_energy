...
for(i in 1:length(cities)){
  ...
  r_df <- r_df %>%
    mutate(JanDec = case_when(JanDec <= -1 & abs(JanDec) >= sd(JanDec) ~ "strong_negative",
                                    JanDec <= -1 & abs(JanDec) < sd(JanDec) ~ "negative",
                                    JanDec >= 1 & abs(JanDec) >= sd(JanDec) ~ "strong_positive",
                                    JanDec >= 1 & abs(JanDec) < sd(JanDec) ~ "positive",
                                    TRUE  ~ "neutral"),
           FebDec = case_when(FebDec <= -1 & abs(FebDec) >= sd(FebDec) ~ "strong_negative",
                                    FebDec <= -1 & abs(FebDec) < sd(FebDec) ~ "negative",
                                    FebDec >= 1 & abs(FebDec) >= sd(FebDec) ~ "strong_positive",
                                    FebDec >= 1 & abs(FebDec) < sd(FebDec) ~ "positive",
                                    TRUE  ~ "neutral"),
           MarDec = case_when(MarDec <= -1 & abs(MarDec) >= sd(MarDec) ~ "strong_negative",
                                    MarDec <= -1 & abs(MarDec) < sd(MarDec) ~ "negative",
                                    MarDec >= 1 & abs(MarDec) >= sd(MarDec) ~ "strong_positive",
                                    MarDec >= 1 & abs(MarDec) < sd(MarDec) ~ "positive",
                                    TRUE  ~ "neutral"),
           AprDec = case_when(AprDec <= -1 & abs(AprDec) >= sd(AprDec) ~ "strong_negative",
                                    AprDec <= -1 & abs(AprDec) < sd(AprDec) ~ "negative",
                                    AprDec >= 1 & abs(AprDec) >= sd(AprDec) ~ "strong_positive",
                                    AprDec >= 1 & abs(AprDec) < sd(AprDec) ~ "positive",
                                    TRUE  ~ "neutral")
...
library(cowplot)
final_plots <- plot_grid(plotlist=plotList[1:5], ncol=1, align="hv")
...
png("combined_plots.png",units="in", width=10, height=10, res=300)
final_plots_legend
dev.off() 
